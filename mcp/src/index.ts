#!/usr/bin/env node
// DO NOT EDIT — generated by _gen/mcp_emitter.py from api_ir.json.
// Regenerate with: cd clients/v2 && python -c "from _gen.mcp_emitter import generate_mcp; generate_mcp()"
...
const GOODMEM_BASE_URL = process.env.GOODMEM_BASE_URL || "http://localhost:8081";
...
async function callApi(
  ...
  const url = new URL(path, GOODMEM_BASE_URL);
  if (query) {
...
function registerTools() {
  ...
  server.tool(
    ...
    api_key: z.any().describe("Configuration when kind is CREDENTIAL_KIND_API_KEY").optional(),
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Structured credential payload describing how to authenticate with the provider. Required for SaaS providers such as COHERE, JINA, and VOYAGE; optional for local or proxy providers.").optional(),
  ...
  server.tool(
    ...
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Structured credential payload describing how to authenticate with the provider").optional(),
  ...
  server.tool(
    ...
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Structured credential payload describing how to authenticate with the provider. Omit for deployments that do not require credentials.").optional(),
      labels: z.record(z.string(), z.string()).describe("User-defined labels for categorization").optional(),
      version: z.string().describe("Version information").optional(),
      monitoring_endpoint: z.string().describe("Monitoring endpoint URL").optional(),
      capabilities: z.object({
    supports_chat: z.boolean().describe("Supports conversational/chat completion format with message roles").optional(),
    supports_completion: z.boolean().describe("Supports raw text completion with prompt continuation").optional(),
    supports_function_calling: z.boolean().describe("Supports function/tool calling with structured responses").optional(),
    supports_system_messages: z.boolean().describe("Supports system prompts to define model behavior and context").optional(),
    supports_streaming: z.boolean().describe("Supports real-time token streaming during generation").optional(),
    supports_sampling_parameters: z.boolean().describe("Supports sampling parameters like temperature, top_p, and top_k for generation control").optional()
  }).describe("LLM capabilities defining supported features and modes. Optional - server infers capabilities from model identifier if not provided.").optional(),
      default_sampling_params: z.object({
    max_tokens: z.number().int().describe("Maximum tokens to generate (>0 if set; provider-dependent limits apply)").optional(),
    temperature: z.number().describe("Sampling temperature 0.0-2.0 (0.0=deterministic, 2.0=highly random)").optional(),
    top_p: z.number().describe("Nucleus sampling threshold 0.0-1.0 (smaller values focus on higher probability tokens)").optional(),
    top_k: z.number().int().describe("Top-k sampling limit (>0 if set; primarily for local/open-source models)").optional(),
    frequency_penalty: z.number().describe("Frequency penalty -2.0 to 2.0 (positive values reduce repetition based on frequency)").optional(),
    presence_penalty: z.number().describe("Presence penalty -2.0 to 2.0 (positive values encourage topic diversity)").optional(),
    stop_sequences: z.any().describe("Generation stop sequences (\u226410 sequences; each \u2264100 chars; generation halts on exact match)").optional()
  }).describe("Default sampling parameters for generation requests").optional(),
  ...
  server.tool(
    ...
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Update credentials").optional(),
    ...
    supports_streaming: z.boolean().describe("Supports real-time token streaming during generation").optional(),
    supports_sampling_parameters: z.boolean().describe("Supports sampling parameters like temperature, top_p, and top_k for generation control").optional()
  }).describe("Update LLM capabilities (replaces entire capability set; clients MUST send all flags)").optional(),
      default_sampling_params: z.object({
    max_tokens: z.number().int().describe("Maximum tokens to generate (>0 if set; provider-dependent limits apply)").optional(),
    temperature: z.number().describe("Sampling temperature 0.0-2.0 (0.0=deterministic, 2.0=highly random)").optional(),
    top_p: z.number().describe("Nucleus sampling threshold 0.0-1.0 (smaller values focus on higher probability tokens)").optional(),
    ...
    presence_penalty: z.number().describe("Presence penalty -2.0 to 2.0 (positive values encourage topic diversity)").optional(),
    stop_sequences: z.any().describe("Generation stop sequences (\u226410 sequences; each \u2264100 chars; generation halts on exact match)").optional()
  }).describe("Update default sampling parameters").optional(),
  ...
  server.tool(
    ...
    memory_id: z.string().describe("Deletes one specific memory by UUID").optional(),
    filter_selector: z.any().describe("Deletes a filtered set of memories within a specific space").optional()
  })).describe("Array of delete selectors")
    },
    async (args) => {
      const body: Record<string, unknown> = {};
      if (args.requests !== undefined) body["requests"] = args.requests;
  ...
  }).describe("Chunking strategy for this memory (if not provided, uses space default)").optional(),
  ...
  })).describe("Optional context items (text or binary) to provide additional context for the search.").optional(),
      space_keys: z.array(z.object({
    space_id: z.string().describe("The unique identifier for the space to search."),
    embedder_weights: z.any().describe("Optional per-embedder weight overrides for this space. If not specified, database defaults are used.").optional(),
    filter: z.string().describe("Optional filter expression that must evaluate to true for memories in this space.").optional()
  })).describe("List of spaces to search with optional per-embedder weight overrides."),
      ...
      hnsw: z.object({
    ef_search: z.number().int().describe("HNSW candidate list size (1..1000).").optional(),
    iterative_scan: z.enum(["ITERATIVE_SCAN_UNSPECIFIED", "ITERATIVE_SCAN_OFF", "ITERATIVE_SCAN_RELAXED_ORDER", "ITERATIVE_SCAN_STRICT_ORDER"]).describe("HNSW iterative scan mode. Use POST retrieve for this advanced tuning control.").optional(),
    max_scan_tuples: z.number().int().describe("Maximum tuples to scan during iterative filtering (1..2147483647).").optional(),
    scan_mem_multiplier: z.number().describe("Multiplier on work_mem for iterative scanning (1.0..1000.0).").optional()
  }).describe("Optional request-level HNSW tuning overrides. Advanced usage; available on POST retrieve.").optional(),
  ...
  }).describe("Optional post-processor configuration to transform retrieval results.").optional()
    },
    async (args) => {
      const body: Record<string, unknown> = {};
  ...
  server.tool(
    ...
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Structured credential payload describing how to authenticate with the provider. Required for SaaS providers; optional for local deployments.").optional(),
      labels: z.record(z.string(), z.string()).describe("User-defined labels for categorization").optional(),
      version: z.string().describe("Version information").optional(),
      monitoring_endpoint: z.string().describe("Monitoring endpoint URL").optional(),
  ...
  server.tool(
    ...
    gcp_adc: z.any().describe("Configuration when kind is CREDENTIAL_KIND_GCP_ADC").optional(),
    labels: z.record(z.string(), z.string()).describe("Optional annotations to aid operators (e.g., \"owner=vertex\")").optional()
  }).describe("Structured credential payload describing how to authenticate with the provider. Omit to keep existing credentials.").optional(),
      replace_labels: z.record(z.string(), z.string()).describe("Replace all existing labels with these (mutually exclusive with mergeLabels)").optional(),
      merge_labels: z.record(z.string(), z.string()).describe("Merge these labels with existing ones (mutually exclusive with replaceLabels)").optional(),
      version: z.string().describe("Version information").optional(),
      monitoring_endpoint: z.string().describe("Monitoring endpoint URL").optional()
  ...
  server.tool(
    ...
    embedder_id: z.string().describe("The unique identifier for the embedder to associate with the space."),
    default_retrieval_weight: z.number().describe("Relative weight for this embedder used by default during retrieval. If omitted, defaults to 1.0; values need not sum to 1 and can be overridden per request.").optional()
  })).describe("List of embedder configurations to associate with this space. Each specifies an embedder ID and a relative default retrieval weight used when no per-request overrides are provided."),
    ...
    recursive: z.any().describe("Recursive hierarchical chunking strategy with configurable separators").optional(),
    sentence: z.any().describe("Sentence-based chunking strategy with language detection").optional()
  }).describe("Default chunking strategy for memories in this space"),
